...
public class LoginServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(user.equals("doctor") && pass == 222)
        {
            request.getRequestDispatcher("/main_doctor.jsp").forward(request, response); 
        }
        else if (user.equals("lab")&& pass ==333)
        {
           request.getRequestDispatcher("/lab_main.jsp").forward(request, response);  
        }
        else if(user.equals("pharm")&& pass==444){
             request.getRequestDispatcher("/pharmain.jsp").forward(request, response);
        }
        else
        {
               ...
               while(itrator.hasNext())
       {
        ...
        {
            out.println("<b><center>Login faild.....</center></b>");
        }
        }
        try {
